        ...
        <div class="col-md-4">
            <div class="card">
                <div class="card-header">
                    <h6 class="card-title mb-0">Role {{ $roles->name ?? '_' }}</h6>
                </div>
                <div class="card-body">
                    <div class="mb-3">
                        <label for="name" class="form-label">Nama Role</label>
                        <input class="form-control" value="{{ $roles->name  ?? '_'  }}" disabled>
                    </div>
                    <div class="mb-3">
                        <label for="dislay_name" class="form-label">Display Nama</label>
                        <input class="form-control" value="{{ $roles->display_name  }}" disabled>
                    </div>
                    <div class="mb-3">
                        <label for="description" class="form-label">Deskripso Nama</label>
                        <textarea class="form-control" disabled>{{ $roles->description }} </textarea>
                    </div>
                </div>
                <div class="card-footer">
                    <a href="{{ route('central-role-page') }}" class="link-success float-end">Back To<i class="ri-arrow-right-s-line align-middle ms-1 lh-1"></i></a>
                    <p class="text-muted mb-0">{{ $roles->created_at->diffForHumans() }}</p>
                </div>
            </div>
        </div>
        <div class="col-md-8">
            <div class="card">
                <div class="card-header bg-light pb-2">
                    <h5 class="card-title mb-0">Role {{ $roles->name ?? '_' }}</h5>
                    <div class="row align-items-center g-2 mt-2 mb-2">
                        
                        {{-- <div class="col-md-auto ms-auto">
                            <div class="hstack gap-2 mt-2 mt-md-0">
                                <label for="name" >Ceklis Semua <span class="text-danger">*</span></label>
                                <input class="form-check-input me-1" type="checkbox" name="name" id="checkAllPermissions">
                            </div>
                        </div><!--end col--> --}}
                    </div><!--end row-->
                </div>
        ...
        $(function(){
            ...
            headers: {
                ...
                if($(this).is(':checked')){
                    ...
                    $.ajax({
                        type:'POST',
                        url: '{{ route('central-permission-attach', $roles->id) }}',
                        data: {
                    ...
                    $.ajax({
                        type:'POST',
                        url: '{{ route('central-permission-detach', $roles->id) }}',
                        data: {
                            permission: $(this).data('permission')
                        },
                        success: function(data){
